...
```ts
type SourceMapExtractType = 'all' | 'js' | 'css';
  ...
  | {
      ...
      extract?:
        ...
        | {
            ...
            /** @deprecated Use `type`, `test`, `include`, and `exclude` instead. */
            js?:
              | boolean
              | {
                  include?: Rspack.RuleSetCondition[];
...
Use `sourceMap.extract` to extract existing source maps from matched files. This is useful when a third-party package already ships both output files and source map files, such as `.js` and `.js.map`.
...
`extract: true` is equivalent to `extract: { type: 'js' }`.

Use a string shorthand or the `type` field to choose which files to match:
...
- `'js'`: extract source maps from JavaScript files. This is the default value.
- `'css'`: extract source maps from CSS files.
- `'all'`: extract source maps from JavaScript and CSS files.
...
Use `test` only when you need a custom file matcher:
...
The old `extract.js` syntax is deprecated and will be removed in a future major version:
...
Use flat `type`, `test`, `include`, and `exclude` fields instead.
:::
