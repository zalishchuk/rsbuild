  ...
  | {
      ...
      extract?:
        ...
        | {
            ...
            /** @deprecated Use `test`, `include`, and `exclude` instead. */
            js?:
              | boolean
              | {
                  include?: Rspack.RuleSetCondition[];
...
:::note
CSS source maps are handled by the CSS build pipeline when `output.sourceMap.css` is enabled. In most cases, you do not need `sourceMap.extract` to preserve existing CSS source maps.
:::
...
When `extract` is enabled, Rsbuild extracts existing source maps from JavaScript files by default.
...
Use flat `test`, `include`, and `exclude` fields instead.
:::
