  ...
  | {
      ...
      extract?:
        ...
        | {
            ...
            /** @deprecated 请使用 `type`、`test`、`include` 和 `exclude`。 */
            js?:
              | boolean
              | {
                  include?: Rspack.RuleSetCondition[];
...
`extract: true` 等价于 `extract: { type: 'js' }`。

使用字符串简写或 `type` 字段选择需要匹配的文件类型：
...
- `'js'`：从 JavaScript 文件中提取 source map，这是默认值。
- `'css'`：从 CSS 文件中提取 source map。
- `'all'`：从 JavaScript 和 CSS 文件中提取 source map。
...
:::warning 已废弃的写法
旧的 `extract.js` 写法已废弃，并会在未来的主版本中移除：
...
请改用扁平的 `type`、`test`、`include` 和 `exclude` 字段。
:::
